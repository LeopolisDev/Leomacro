...
SendInfo(matchResult := "") {
    ...
    Play_Again := AdvImageSearch(TriumphImg2, Integer(w * 0.2), Integer(h * 0.2), Integer(w * 0.6), Integer(h * 0.7), 0.5, 1.5, 0.02)
    Restart := AdvImageSearch(RestartImg, Integer(w * 0.2), Integer(h * 0.2), Integer(w * 0.6), Integer(h * 0.7), 0.5, 1.5, 0.02)
    Restart2 := AdvImageSearch(RestartImg2, Integer(w * 0.2), Integer(h * 0.2), Integer(w * 0.6), Integer(h * 0.7), 0.5, 1.5, 0.02)
    ...
    if (SendCurrenciesEnabled = "1") {
        ...
        if RegExMatch(ocrInfo, "i)(\d+\s*m(?:\s*[\dlIi|]+\s*s)?)", &timeMatch) {
            timeCompleted := RegExReplace(timeMatch[1], "i)[lIi|]", "1")
            timeCompleted := RegExReplace(timeCompleted, "\s+", " ") 
        } else if RegExMatch(ocrInfo, "i)(\d+\s*s)", &timeMatch) {
            timeCompleted := Trim(timeMatch[1])
        }
        ...
        if RegExMatch(timeCompleted, "(\d+)m", &mMatch)
            timeInSeconds += Integer(mMatch[1]) * 60
        if RegExMatch(timeCompleted, "(\d+)s", &sMatch)
            timeInSeconds += Integer(sMatch[1])
